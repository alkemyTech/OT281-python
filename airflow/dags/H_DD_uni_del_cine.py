...
################## DAG INIT #############################
with DAG(
    dag_id="H_DD_H_uni_del_cine",
    default_args=default_args,
    description='Consulta a H_uni_del_cine',
    ...
    # 0 - init Log
    logger.debug("H_uni_del_cine start")

    # 1 - Data from postgres database
    udc_extract_data = PythonOperator(
        task_id='extract_data_H_uni_del_cine',
        python_callable=extract_data
    )

    # 2 - Transforms Data
    udc_pandas_transform = PythonOperator(
       task_id='transform_data_H_uni_del_cine',
    ...
    udc_load_data=PythonOperator(
                  task_id='load_data_H_uni_del_cine',
