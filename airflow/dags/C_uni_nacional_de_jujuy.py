...
def open_csv_to_pd():
    ...
    file_name_csv_clean=file_name_csv
    # Add path to the file with the file name in str format
    ...
    C_universidades_pd = pd.read_csv(filename)
    # Edit and transform data in DF and output a new DF
    new_df = create_export_dataframe(C_universidades_pd)
...
# Set DAG
with DAG(
    "DD_C_uni_nacional_de_jujuy",
    schedule_interval="@hourly",
    start_date=datetime(year=2022, month=8, day=22),
    default_args=default_args,
    catchup=False
) as dag:
    logger.debug("DD_C_uni_nacional_de_jujuy")
    
# Define task to query and donwload data to csv
    task_universidades_load_query = PythonOperator(
    	task_id='universidades_load_query',
    	python_callable=get_postgress_data,
    )
# Define task to open csv and transform into DataFrame
    task_universidades_csv_to_pd = PythonOperator(
        task_id='universidades_csv_to_pd',
    ...
    task_C_local_to_s3_job = PythonOperator(
        ...
        op_kwargs={
            'filename':    "{{ ti.xcom_pull(task_ids='universidades_csv_to_pd')}}",
...
# SET AIRFLOW FLOW PROCESS 
    task_universidades_load_query >> task_universidades_csv_to_pd >> task_C_local_to_s3_job
